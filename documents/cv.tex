...
\lskip
\topic{Telephone} +61 4 1164 8655
\topic{Address} Sydney, NSW, Australia
\topic{E-mail} dezwart@gmail.com
\pskip

\heading{Key Strengths}

\dashtopic Highly proficient with {\mb Linux/Unix} environments.
\lskip
\dashtopic Designing solutions to problems \& following them through to completion, from an inherent desire to improve the status quo.
\dashtopic Technical documentation, from writing change control requests, installation documentation \& operations procedures.
\dashtopic Take complex topics \& explain them in simple terms, learnt through academic tutoring.
\dashtopic Researching topics so that new knowledge can be applied to current projects.
\pskip
...
\lskip
\topic{Role} Performance Engineer
\topic{Dates} 01/2012 - Present
\topic{Reported to} Performance Engineering Lead
\topic{Achieved} Oracle Java SE HotSpot GC tuning guide\footnoter{https://confluence.atlassian.com/display/ATLAS/Garbage+Collection+(GC)+Tuning+Guide}, with a focus on Atlassian products.
\dashtopic JIRA performance test harness\footnoter{https://confluence.atlassian.com/display/ATLAS/JIRA+Performance+Testing+with+Grinder} to work around lack of automated load generation clustering with Grinder.
\dashtopic Deep code diving in to the Oracle HotSpot compiler C++ source after observing behaviour I hadn't seen previously with the code cache, documenting my findings via a blog post\footnoter{http://blogs.atlassian.com/2012/05/codecache-is-full-compiler-has-been-disabled}.
\dashtopic Design and rollout of a modular performance lab. 
\topic{Duties} Systems \& application performance testing.
...
\lskip
\topic{Dates} 06/2010 - 01/2012
\topic{Reported to} Systems Engineering Lead
\topic{Achieved} Developed tools to automate host \& service discovery, including dependency correlation.
...
\topic{Duties} Planning \& executing projects around Atlassian's  critical production systems.
\dashtopic Monitoring, maintaining \& optimising those systems.
\dashtopic Working closely with developers \& other staff to solve operational issues with Atlassian's applications.
\dashtopic Scheduling, co-ordinating \& communicating maintenance windows for critical systems.
\dashtopic Co-ordination with Atlassian's colocation providers.
\dashtopic Ensuring the security of Atlassian's critical systems.
\topic{Reason left} Opportunity to join Performance Engineering team.
\pskip
...
\topic{Common Duties} Senior First Aid Officer
\lskip
...
\dashtopic Engaging with Dot's suppliers to ensure continuity \& quality of service to reach SLA KPIs.
